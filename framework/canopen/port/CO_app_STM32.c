...
/* [本项目新增] 上游在 CO_RESET_COMM 时用 HAL_TIM_Base_Stop_IT() 停掉 1ms 中断，
 * 避免中断在 CO_delete() 释放 CO 之后访问它。本工程 1ms 时基来自 SysTick，
 * 无法停止中断，故用此标志达到同样的互斥。详见 PATCHES.md。
 * volatile：主循环(canopen_app_process)与中断(canopen_app_interrupt)都会改写。 */
...
canopen_app_resetCommunication() {
    ...
    /* [本项目已改] 原为 HAL_TIM_Base_Start_IT(timerHandle)。1ms 时基改由 SysTick
     * 提供，无需启动硬件定时器：canopen_app_interrupt() 由 main.c 的
     * SysTick_Handler 每毫秒调用。详见 PATCHES.md */
    canopenRunning = true; /* 此后允许 canopen_app_interrupt() 访问 CO */
...
canopen_app_process() {
    ...
    if ((time_current - time_old) > 0) { // Make sure more than 1ms elapsed
        ...
        if (reset_status == CO_RESET_COMM) {
        	...
        	canopenRunning = false; /* [本项目已改] 先禁止中断访问，再释放 CO */
            CO_CANsetConfigurationMode((void*)canopenNodeSTM32);
            CO_delete(CO);
...
canopen_app_interrupt(void) {
    /* [本项目新增] CO 尚未就绪、或正在 CO_RESET_COMM 中重建时不得访问，
     * 否则会 use-after-free。见 canopenRunning 的说明与 PATCHES.md */
    if (!canopenRunning) {
        return;
    }
